...
def _identify_personas(reviews: List[Dict]) -> Tuple[List[Dict], str]:
    ...
    for dim_name, tag_keys in dimension_attempts:                       # 按优先级依次尝试
        ...
        for review in reviews:                                           # 遍历所有评论
            ...
            """
            假设有 2 条评论属于 "家用_女性" 这个画像：

            评论1 的 tags：
            {"人群_性别": "女性", "场景_使用场景": "家用", "质量_做工": "精细", "体验_舒适度": "舒适"}
            评论2 的 tags：
            {"人群_性别": "女性", "场景_使用场景": "家用", "质量_做工": "精细", "体验_舒适度": "一般"}

            代码跑完后，persona_tag_stats 长这样：
            {
                "家用_女性": {                          # 画像名
                    "人群_性别":  Counter({"女性": 2}),      # 2个人都是女性
                    "场景_使用场景": Counter({"家用": 2}),    # 2个人都是家用
                    "质量_做工":   Counter({"精细": 2}),      # 2个人都说做工精细
                    "体验_舒适度":  Counter({"舒适": 1, "一般": 1})  # 1个说舒适，1个说一般
                }
            }

            然后第 168 行 counter.most_common(1)[0] 取每个标签下出现最多的值，最终得到这个画像的"典型特征"：
            typical_tags = {
                "人群_性别":  "女性(仅2人有此特征)",
                "场景_使用场景": "家用(仅2人有此特征)",
                "质量_做工":   "精细(仅2人有此特征)",
                "体验_舒适度":  "舒适(仅1人有此特征)"   # 1:1 平局时取先出现的
            }

            本质就是：把属于同一个画像的所有评论的标签汇总，找出每个维度上最常出现的值，作为这个画像的"群体特征名片"。
            """

            """
            抛开代码，用大白话说：

            你在学校门口做调研，问了 10 个同学"为什么喜欢这家奶茶店"。
            你先把人分成两组："女生_学生" 和 "男生_上班族"。

            然后你翻看 "女生_学生" 这组所有人的回答，发现：
            8 个人提到了"好喝"
            6 个人提到了"便宜"
            3 个人提到了"包装好看"
            那这一组的群体特征就是：注重口味和价格。

            这段代码干的就是这个事——把同一个画像的所有人的标签汇总，找出出现频率最高的那几个词，作为这个画像的"群体名片"。

            persona_tag_stats["女生_学生"]["好喝"] += 1   # 又一个人说好喝，计数+1
            
            最后把计数最多的取出来："好喝 (8/10人提到)"。这就是这个画像的典型特征。
